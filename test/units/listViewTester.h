...
namespace tester {
	...
	public:
		listViewTester() : utils::TestSuite("set Tester") {
			add_test("Construct Basic", "set anchors at requested start", test_construct_basic);
			...
			add_test("Subview Basics", "subSet reanchors window correctly", test_subview_basics);
			add_test("Subview Out Of Range", "subSet rejects start beyond size", test_subview_out_of_range);
			add_test("Iteration Helpers", "begin/endIter span the active view", test_iteration_helpers);
		}

	private:
		using IntList = std::vector<int>;
		using View = ::utils::set<IntList>;
